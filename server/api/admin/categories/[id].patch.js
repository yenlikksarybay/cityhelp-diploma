...
export default defineEventHandler(async (event) => {
	...
	category.description = String(body?.description ?? category.description).trim();
	category.subcategories = normalizeSubcategories(body?.subcategories ?? category.subcategories);
	category.order = Number(body?.order ?? category.order);
	category.isActive = Boolean(body?.isActive ?? category.isActive);
	if (body?.key) {
		category.key = String(body.key).trim();
	}
	await category.save();
