...
export default defineEventHandler(async (event) => {
	...
	const employee = await UserModel.findOne({ _id: id, role: "employee" }).lean();
	if (!employee) {
		throw createError({ statusCode: 404, statusMessage: "Сотрудник не найден" });
	}

	const filter = { assignedEmployee: employee._id };
	if (statusGroup) {
	...
	const stat = stats?.[0] || {};
	const employeePayload = sanitizeUser(employee);

	return createSuccessResponse({
		message: "Сотрудник получен",
		data: {
			employee: {
				...employeePayload,
			},
